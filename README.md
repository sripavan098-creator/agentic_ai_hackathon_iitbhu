...
```
*This creates realistic fake PDFs in the `mock_data/` folder: an EOB denial letter, an insurance policy, and a hospital directory.*
...
```
*Opens a beautiful web interface at `http://localhost:8501` where you can upload PDFs, watch the agent work in real-time, and trigger the insurance pushback adaptation.*

#### Option B: Command Line (For Testing Agent Logic)
```bash
python main.py
```
*Runs the full agent workflow in the terminal, including the simulated insurance pushback and adaptation loop.*
...
```text
mediadvocate/
├── app.py                  # Streamlit frontend UI (interactive demo)
├── main.py                 # CLI entry point for agent execution
├── generate_mock_data.py   # Generates realistic fake PDFs for testing
├── requirements.txt        # Python dependencies
├── .env.example            # Template for environment variables
├── .gitignore              # Ensures .env and local DBs are not committed
├── agent/
│   ├── __init__.py         # Package init
│   ├── state.py            # AgentState TypedDict (agent memory)
│   ├── graph.py            # LangGraph cyclic state machine
│   ├── nodes.py            # Agent nodes (Observe, Act, Draft, Evaluate, Adapt)
│   └── tools.py            # Tool definitions (Tavily, ChromaDB, PyPDF)
├── mock_data/              # Sample PDFs (generated by generate_mock_data.py)
│   ├── eob_denial.pdf      # Fake EOB denial letter
│   ├── insurance_policy.pdf # Fake insurance policy (Page 42)
│   └── hospital_directory.pdf # Fake hospital network directory
└── README.md               # This file
```
